...
export class AppComponent {
  ...
  drawLine(source, featur) {
    ...
    self.draw.on("drawend", function(event) {
      ...
      if (currentSelected.length) {
        ...
        console.log(selectedCoordinatres, "//");
        let poly = [selectedCoordinatres];
        let options = {
          shortestPath: turf.polygon({
            poly
          })
        };
        ...
        // 判断点是否在直线上
        // let startPointOnline = self.pointOnline(startPoint, lineStart, lineEnd)
        // let endPointOnline = self.pointOnline(endPoint, lineStart, lineEnd)
        // return;
      ...
      features: [
        {
          ...
          geometry: {
            ...
            coordinates: [
              [
                [117.10743963718414, 40.19390257315284],
                [117.10743963718414, 40.19390257315284],
                [117.10706412792206, 40.19336235891643],
                [117.10704803466797, 40.19335621233272],
                [117.10705071687698, 40.19335928562465],
                [117.10671973228453, 40.19265303947918],
                [117.10670471191406, 40.19256534724838],
                [117.10676550865175, 40.19241099771932],
                [117.10798501968382, 40.19264866852956],
                [117.1082046031952, 40.193215250526805],
                [117.10882544517519, 40.19373593356925],
                [117.10886657238007, 40.194173020130094],
                [117.10758984088898, 40.19436151284005],
                [117.10806280374526, 40.19366422378636]
              ]
            ]
          }
        }
  ...
  doFusion(layer, features, e) {
    const polygon1 = features[0][0].getGeometry();
    const polygon2 = features[1][0].getGeometry();

    //feature.getGeometry().getCoordinates()
    const parse = arr => {
      const toStr = arr.map(v => v.join(" ")).join(",");
      return `POLYGON((${toStr}))`;
    };

    //console.log(polygon1,polygon2);
    //console.log(new JstsIo.WKTParser(polygon1));

    const reader = new JstsIo.WKTReader();
    //const a = reader.read('POLYGON((10 10, 100 10, 100 100, 10 100, 10 10))');
    //const b = reader.read('POLYGON((50 50, 200 50, 200 200, 50 200, 50 50))');
    polygon1.scale(2, 2);
    polygon2.scale(2, 2);
    console.log(parse(polygon1.getCoordinates()[0]), "//");

    const a = reader.read(parse(polygon1.getCoordinates()[0]));
    const b = reader.read(parse(polygon2.getCoordinates()[0]));
    const cls = {
      geom: {
        Polygon
      }
    };
    const parser = new JstsIo.OL3Parser(undefined, {
      geom: {
        Polygon
      }
    });
    let union = a.union(b);
    const polygon = parser.convertToPolygon(union);
    ...
    let union = a.union(b);
    console.log(union);
    console.log(union.getGeometryN());
    const polygon = features[0][0].getGeometry();
    polygon.union()
     */
  ...
  getNearByIndex(arr, nearArr) {
    ...
    //   if (arr[i] === nearArr[0]) {
    //     start = i;
    //   } else if (arr[i] === nearArr[nearArr.length - 1]) {
    //     end = i;
    //   } else {
    //     arr.splice(i, 1);
    //   }
    // }
    start = arr.indexOf(nearArr[0]);
    end = arr.indexOf(nearArr[nearArr.length - 1]);
    nearArr.forEach((item, key) => {
      if (key !== 0 && key !== nearArr.length - 1) {
        arr.splice(key, 1);
      }
    });
  ...
  refetchArray(obj, obj2) {
    ...
    let arr2 = obj2.arr;
    let newArr = [];
    let polygon1 = this.filterPolygon1(obj);
    let polygon2 = this.filterpolygon2(obj2);
    // TODO: 处理合并块数据截取、重组

    newArr = polygon1.arrFront
      .concat(polygon2.arrFront)
      .concat(polygon2.arrEnd)
      .concat(polygon1.arrEnd);
    return newArr;
  }
  filterPolygon(obj) {
    let arr = obj.arr;
    ...
    // 在之前 保留第一个与最后一个项目的坐标，其他移除
    // start end 需是移除相邻坐标后的索引

    if (obj.start === 0) {
      arrFront = arr.slice(obj.start, obj.end);
      arrEnd = obj.end < arr.length ? arr.slice(obj.end, arr.length) : [];
      // 第一个坐标索引为0
    } else if (obj.start < obj.end) {
      // 0 < obj.start < obj.end
      // 判断obj.end < arr.length ?
      arrFront = arr.slice(0, obj.start);
      arrEnd = arr.slice(obj.start, obj.end);
      arrThree = obj.end < arr.length ? arr.slice(obj.end, arr.length) : [];
    } else {
      // 0 < obj.start > obj.end
      // 判断 obj.start =< arr.length?
    }
    return { arrFront, arrEnd, arrThree };
  }
  filterPolygon1(obj) {
    let arr = obj.arr;
    let arrFront = [];
    ...
    if (obj.start < obj.end) {
      let endIndex = obj.end + 1 < arr.length ? obj.end + 1 : obj.end;
      arrFront = arr.slice(obj.start, endIndex);
      arrEnd = arr.slice(endIndex, arr.length);
      if (obj.start > 0) {
        let temp = arr.slice(0, obj.start);
        arrEnd.concat(temp);
      }
    } else {
      let endIndex = obj.end + 1 < arr.length ? obj.end + 1 : obj.end;
      arrFront = arr.slice(obj.start, endIndex);
      arrEnd = arr.slice(0, endIndex);
      let temp = arr.slice(endIndex, obj.start - 1);
      if (temp.length) {
        arrEnd.concat(temp);
      }
    }
  ...
  filterpolygon2(obj) {
    ...
    let arrEnd = [];
    let endIndex = obj.end + 1 < arr.length ? obj.end + 1 : obj.end;
    let startIndex = obj.start + 1;

    if (obj.start < obj.end) {
      arrFront = arr.slice(startIndex, obj.end);
      arrEnd = arr.slice(endIndex, arr.length);
      if (obj.start > 0) {
        let temp = arr.slice(0, obj.start);
        arrEnd.concat(temp);
      ...
      arrFront = arr.slice(endIndex, obj.start);
      arrEnd = arr.slice(obj.start, arr.length);
      if (obj.end > 0) {
        let temp = arr.slice(0, obj.end);
        if (temp.length) {
          arrEnd.concat(temp);
        }
      }
